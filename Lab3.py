...
def analisis_exploratorio(df, nombre):
    ...
    for col in df.columns:
        ...
        print(
            f"   {col}: {unique_vals} unicos / {total_vals} totales ({porcentaje:.2f}% unicos)"
        )
        if unique_vals <= 20:
    ...
    if len(num_cols) >= 2:
        ...
        for i in range(len(corr_matrix.columns)):
            for j in range(i + 1, len(corr_matrix.columns)):
                ...
                if abs(corr_val) > 0.5:
                    print(
                        f"   {corr_matrix.columns[i]} y {corr_matrix.columns[j]}: {corr_val:.3f}"
                    )
    ...
    for col in cat_cols:
        ...
        # Detectar telefonos
        patron_telefono = (
            r"^\+?\d{1,3}[-.\s]?\(?\d{2,4}\)?[-.\s]?\d{3,4}[-.\s]?\d{4}$|^\d{10,15}$"
        )
        ...
        if 2 <= df[col].nunique() <= 15:
            ...
            if len(num_cols) > 0:
                for num_col in num_cols[:3]:
                    ...
                    grouped_stats = df.groupby(col)[num_col].agg(
                    ...
                    if len(grouped_stats) >= 2:
                        ...
                        if min_mean > 0 and (max_mean / min_mean) > 3:
                            print(
                                f"   Diferencia significativa detectada en '{num_col}' (ratio: {max_mean / min_mean:.2f})"
                            )
